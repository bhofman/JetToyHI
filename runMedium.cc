...
int main (int argc, char ** argv) {
  ...
  while ( mixer.next_event() && iev < nEvent )
  {
    ...
    jetCollection jetCollectionSig(sorted_by_pt(jet_selector(csSig.inclusive_jets(user_pt))));

    fastjet::ClusterSequenceArea csSigSub(subtracted_particles, jet_def, area_def);
    jetCollection jetCollectionCSSub(sorted_by_pt(jet_selector(csSigSub.inclusive_jets(10.)))); // Inclusive jets to take a jets with pt over (pt_min)
